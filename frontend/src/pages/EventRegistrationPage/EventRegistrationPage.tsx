...
const EventRegistrationPage: React.FC = () => {
    ...
    const validateTeamName = (showErrors: boolean = false): boolean => {
        ...
        if (/[^\w\s]/.test(teamName)) {
            if (showErrors) toast.error('Nama tim tidak boleh mengandung karakter spesial (., dll)');
            return false;
        }
    ...
    const goToNextStep = () => {
        if (currentStep === 'team-name') {
            ...
            if (!validateMemberInfo(creatorDetails, true)) { // Show error toasts when trying to proceed
                return;
            }
            
            // Validate team member info if any members added
            for (let i = 0; i < teamMembers.length; i++) {
                if (!validateMemberInfo(teamMembers[i], true)) { // Show error toasts when trying to proceed
                    return;
                }
            }
    ...
    const canNavigate = (fromStep: FormStep, toStep: FormStep): boolean => {
        ...
        if (fromStep === 'members-info' && toStep === 'payment') {
            ...
            for (const member of teamMembers) {
                if (!validateMemberInfo(member, false)) { // Don't show error toasts during navigation checks
                    membersValid = false;
                    break;
                }
            }
    ...
    const handleNavigateToStep = (step: FormStep) => {
        ...
        if (step === 'members-info') {
            if (currentStep === 'payment' || canNavigate('team-name', 'members-info')) {
                setCurrentStep('members-info');
                return;
            }
        }
        ...
        <div className={styles.container}>
            ...
            <div className={styles.formContainer}>
                ...
                <div className={styles.header}>
                    {currentStep === 'team-name' && (
                        <>
                            ...
                            <p className={styles.formSubtitle}>Masukkan nama tim dan pastikan semua validasi terpenuhi sebelum lanjut.</p>
                        </>
                    )}
                    {currentStep === 'members-info' && (
                        <>
                            <h2 className={styles.formTitle}>PENDATAAN TIM ANGGOTA</h2>
                            <p className={styles.formSubtitle}>Isi data lengkap seluruh anggota tim, termasuk nama, email, kontak, dan institusi.</p>
                        </>
                    )}
                    {currentStep === 'payment' && (
                        <>
                ...
                <div className={styles.progressContainer}>
                    <div 
                        className={`${styles.progressStep} ${currentStep === 'team-name' || currentStep === 'members-info' || currentStep === 'payment' ? styles.active : ''}`} 
                        onClick={() => handleNavigateToStep('team-name')}
                        style={{ cursor: 'pointer' }}
                    >
                        <Pencil style={{ width: '16px', height: '16px' }} />
                    </div>
                    <div className={`${styles.progressLine} ${currentStep === 'members-info' || currentStep === 'payment' ? styles.completed : ''}`}></div>
                    <div 
                        className={`${styles.progressStep} ${currentStep === 'members-info' || currentStep === 'payment' ? styles.active : ''}`} 
                        onClick={() => canNavigate('team-name', 'members-info') && handleNavigateToStep('members-info')}
                        style={{ cursor: canNavigate('team-name', 'members-info') ? 'pointer' : 'not-allowed' }}
                    >
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    {currentStep === 'members-info' && (
                        <>
                            ...
                            <div className={styles.memberSection}>
                                ...
                                <div className={styles.formGroup}>
                                    <label htmlFor="leaderName">Nama Lengkap <span className={styles.required}>*</span></label>
                                    <input 
                                        type="text" 
                                        id="leaderName" 
                                ...
                                <div className={styles.formGroup}>
                                    <label htmlFor="leaderEmail">Email <span className={styles.required}>*</span></label>
                                    <input 
                                        type="email" 
                                        id="leaderEmail" 
                                ...
                                <div className={styles.formGroup}>
                                    <label htmlFor="leaderDob">Tanggal Lahir <span className={styles.required}>*</span></label>
                                    <div className={styles.dateInput}>
                                        <input 
                                            type="date" 
                                            id="leaderDob" 
                                            value={creatorDetails.dateOfBirth} 
                                            onChange={(e) => handleCreatorChange('dateOfBirth', e.target.value)} 
                                            required
                                            placeholder="dd/mm/yy"
                                        />
                                        <svg className={styles.dateIcon} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M8 2V5M16 2V5M3.5 9.09H20.5M21 8.5V17C21 20 19.5 22 16 22H8C4.5 22 3 20 3 17V8.5C3 5.5 4.5 3.5 8 3.5H16C19.5 3.5 21 5.5 21 8.5Z" stroke="white" strokeOpacity="0.5" strokeWidth="1.5" strokeMiterlimit="10" strokeLinecap="round" strokeLinejoin="round"/>
                                        </svg>
                                    </div>
                                </div>
                                <div className={styles.formGroup}>
                                    ...
                                    <div className={styles.radioGroup}>
                                        <label
                                            className={`${styles.radioBox} ${creatorDetails.gender === 'Laki-Laki' ? styles.selected : ''}`}
                                        >
                                            <input
                                                type="radio"
                                                name="gender"
                                                ...
                                                onChange={(e) => handleCreatorChange('gender', e.target.value)} 
                                                required
                                            />
                                        ...
                                        <label
                                            className={`${styles.radioBox} ${creatorDetails.gender === 'Perempuan' ? styles.selected : ''}`}
                                        >
                                            <input
                                                type="radio"
                                                name="gender"
                                ...
                                <div className={styles.formGroup}>
                                    <label htmlFor="leaderWhatsapp">Nomor WhatsApp <span className={styles.required}>*</span></label>
                                    <input 
                                        type="tel" 
                                        id="leaderWhatsapp" 
                                        value={creatorDetails.whatsappNumber} 
                                        onChange={(e) => handleCreatorChange('whatsappNumber', e.target.value)} 
                                        required 
                                    />
                                </div>
                                <div className={styles.formGroup}>
                                    <label htmlFor="leaderInstitution">Institusi / Asal Sekolah <span className={styles.required}>*</span></label>
                                    <input 
                                        type="text" 
                                        id="leaderInstitution" 
                                        value={creatorDetails.institution} 
                                        onChange={(e) => handleCreatorChange('institution', e.target.value)} 
                                        required 
                                    />
                                </div>
                                <div className={styles.formGroup}>
                            ...
                            {teamMembers.map((member, index) => (
                                <div key={index} className={styles.memberWrapper}>
                                    <div className={styles.memberSection}>
                                        <div className={styles.memberHeader}>
                                            <h3 className={styles.sectionTitle}>Anggota Kelompok {index + 1}</h3>
                                            <button 
                                                type="button" 
                                    ...
                                    <div className={styles.formGroup}>
                                        ...
                                        <div className={styles.radioGroup}>
                                            <label
                                                className={`${styles.radioBox} ${member.gender === 'Laki-Laki' ? styles.selected : ''}`}
                                            >
                                                <input
                                                    type="radio"
                                            ...
                                            <label
                                                className={`${styles.radioBox} ${member.gender === 'Perempuan' ? styles.selected : ''}`}
                                            >
                                                <input
                                                    type="radio"
                                                    name={`memberGender-${index}`}
                                    ...
                                    <div className={styles.formGroup}>
                                        <label>Nomor WhatsApp <span className={styles.required}>*</span></label>
                                        <input 
                                            type="tel" 
                                    ...
                                    <div className={styles.formGroup}>
                                        <label>Institusi / Asal Sekolah <span className={styles.required}>*</span></label>
                                        <input 
                                            type="text" 
                                    ...
                                    <div className={styles.formGroup}>
                                        <label>Upload Kartu Mahasiswa / Pelajar <span className={styles.required}>*</span></label>
                                        <div className={styles.fileUploadContainer}>
                                            <input 
                                                type="file" 
                                            ...
                                            <label htmlFor={`member${index}IdCard`} className={styles.uploadBtn} onClick={(e) => {
                                                e.preventDefault();
                                                document.getElementById(`member${index}IdCard`)?.click();
                                            }}>
                                                <Upload style={{ width: '16px', height: '16px' }} />
                                                Add File
                                            </label>
                            ...
                            >
                                {paymentFile ? (
                                    ...
                                    <>
                                        ...
                                        <p className={styles.dragDropHint}>Supported file types: .png, .jpg, and other image formats.</p>
                                        <p className={styles.dragDropHint}>Maximum file size: 5 MB</p>
                                    </>
                                )}
                                <input 
                                    type="file" 
                    ...
                    <div className={styles.buttonContainer}>
                        {currentStep !== 'payment' ? (
                            ...
                            >
                                {isSubmitting ? <div className={styles.spinner}></div> : 'Daftarkan Tim!'}
                            </button>
                        )}
                    </div>
                </form>
